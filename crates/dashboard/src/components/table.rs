...
pub fn data_grid(props: &Props) -> Html {
                ...
                let new_filtered_data = data
                    ...
                    .filter(|beer| {
                        HEADERS.iter().enumerate().all(|(index, &(header, _))| {
                            let filter = &lower_filters[index];
                            filter.is_empty() || beer.filter_check(header, &filter)
                        })
                    })
                    .collect::<IArray<_>>();
    ...
    let on_sort = {
        ...
        Callback::from(move |(index, header): (usize, &str)| {
            let new_ascending = match *sort_by {
                Some(current_sort) if current_sort == index => !*ascending,
                _ => true,
            };
        ...
        .map(|(index, &(header, input_type))| {
            ...
            let header_classes = match *sort_by {
                Some(current_sort) if current_sort == index => {
                    classes!(
                        "col-title",
                        if *ascending { "sort-asc" } else { "sort-desc" }
                    )
                }
                _ => classes!("col-title"),
            };
